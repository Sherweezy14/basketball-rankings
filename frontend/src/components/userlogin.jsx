...
function UserLogin() {
  ...
  function checkValidation(event) {
    if (
      regexTest[event.target.name].test(event.target.value) ||
      event.target.value === ""
    ) {
  ...
  return !token ? (
    <>
      ...
      >
        ...
        <button
          ...
          className="bg-purple-700 text-white font-bold px-4 py-2 rounded-xl disabled:bg-slate-300 disabled:text-slate-500 disabled:hover:bg-slate-300 disabled:cursor-not-allowed"
          type="Submit"
        >
          {" "}
          Login{" "}
    ...
    <div className="flex flex-col items-center gap-2">
      <p className="hidden text-2xl lg:block">
        {" "}
        {"Hello " + userLoggedIn.name.split(" ")[0]}
      </p>
      <p className="text-red-500 mt-5 text-1xl lg:hidden" onClick={logOutUser}>
        {" "}
        Sign Out
      </p>
      <button
        className="hidden lg:block bg-red-700 text-white font-bold px-3 py-1.5 rounded-xl"
        onClick={logOutUser}
      >
        {" "}
        LogOut{" "}
